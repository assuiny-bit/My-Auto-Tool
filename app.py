...
# =================================================================
# C 強化版 V4.1：步驟 12 調整為中心點擊 + 完整循環功能
# =================================================================
...
class MainApp:
    ...
    def run_steps(self):
        ...
        try:
            ...
            bx = (loc.left + loc.width // 2) + 32 if loc else pyautogui.position().x
            by = (loc.top + loc.height // 2) - 30 if loc else pyautogui.position().y
            move_mouse_to(bx, by); time.sleep(0.8)
        except: bx, by = pyautogui.position().x, pyautogui.position().y
        ...
        # 12. 搜尋 ca2.png 並直接點擊中心 (移除偏移)
        self.status_label.config(text="搜尋 ca2.png...")
        try:
            loc = pyautogui.locateOnScreen(resource_path("ca2.png"), confidence=0.8)
            if loc:
                # 移動到圖片正中心，不加偏移
                tx_ca2 = loc.left + loc.width // 2
                ty_ca2 = loc.top + loc.height // 2
                
                self.status_label.config(text="定位 ca2 成功! 執行中心點擊")
